...
mysql -u root --password=root -e "GRANT ALL PRIVILEGES ON ${DB_NAME}.* TO renato@localhost IDENTIFIED BY 'wp';"
echo -e "\n DB operations done.\n\n"
...
if [[ ! -f "${VVV_PATH_TO_SITE}/wp-config.php" ]]; then
  ...
  noroot wp core config --dbname="${DB_NAME}" --dbuser=renato --dbpass=wp --quiet --extra-php <<PHP
